...
The selected model, [`nvidia/nemotron-3-super-120b-a12b`](https://build.nvidia.com/nvidia/nemotron-3-super-120b-a12b), is available through NVIDIA's free API endpoint for prototyping. Account access and service limits still apply.

For local file-based configuration, keep the values in the repository-root `.env` (ignored by Git), not `.env.example`. From `backend/`, load your own local file before starting the service:
