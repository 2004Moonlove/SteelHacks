...
- **Complete your comparison**: generated choices open a guide for missing actual values and proposed factors awaiting confirmation. Enter the horizon, matching weekly frequency, prices and durations; add a billing interval if it is missing. Confirm applicable factors to use them. Subsequent edits recalculate immediately without model calls. Reference-only qualitative factors may stay unknown. A budget is a limit, not an option price.
